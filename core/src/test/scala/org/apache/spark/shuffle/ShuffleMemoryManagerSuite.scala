...
class ShuffleMemoryManagerSuite extends FunSuite with Timeouts {
   ...
   */
  override def spillMySelf: Unit = Unit

    override protected var elementsRead: Long = 0L
  }
  ...
  test("single thread requesting memory") {
    withSparkContext { sc =>
      ...
      assert(manager.tryToAcquire(asker, 100L) === 100L)
      assert(manager.tryToAcquire(asker, 400L) === 400L)
      assert(manager.tryToAcquire(asker, 400L) === 400L)
      assert(manager.tryToAcquire(asker, 200L) === 100L)
      assert(manager.tryToAcquire(asker, 100L) === 0L)
      assert(manager.tryToAcquire(asker, 100L) === 0L)

      manager.release(asker, 500L)
      assert(manager.tryToAcquire(asker, 300L) === 300L)
      assert(manager.tryToAcquire(asker, 300L) === 200L)

      manager.releaseMemoryForThisThread()
      assert(manager.tryToAcquire(asker, 1000L) === 1000L)
      assert(manager.tryToAcquire(asker, 100L) === 0L)
    }
  }
  ...
  test("two threads requesting full memory") {
    withSparkContext { sc =>
      ...
      val t1 = startThread("t1") {
        val r1 = manager.tryToAcquire(asker1, 500L)
        state.synchronized {
          state.t1Result1 = r1
          state.notifyAll()
        ...
        val r2 = manager.tryToAcquire(asker1, 500L)
        state.synchronized {
          state.t1Result2 = r2
        }
      }
      ...
      val t2 = startThread("t2") {
        val r1 = manager.tryToAcquire(asker2, 500L)
        state.synchronized {
          state.t2Result1 = r1
          state.notifyAll()
        ...
        val r2 = manager.tryToAcquire(asker2, 500L)
        state.synchronized {
          state.t2Result2 = r2
        }
      }
  ...
  test("threads cannot grow past 1 / N") {
    ...
    withSparkContext { sc =>
      ...
      val t1 = startThread("t1") {
        val r1 = manager.tryToAcquire(asker1, 250L)
        state.synchronized {
          state.t1Result1 = r1
          state.notifyAll()
          while (state.t2Result1 === -1L) {
      ...
      val t2 = startThread("t2") {
        val r1 = manager.tryToAcquire(asker2, 250L)
        state.synchronized {
          state.t2Result1 = r1
          state.notifyAll()
          while (state.t1Result1 === -1L) {
  ...
  test("threads can block to get at least 1 / 2N memory") {
    ...
    // by t2 will return false right away because it now has 1 / 2N of the memory.
     withSparkContext { sc =>
       ...
       val t1 = startThread("t1") {
         state.synchronized {
           state.t1Result = manager.tryToAcquire(asker1, 1000L)
       ...
       val t2 = startThread("t2") {
         ...
         val result = manager.tryToAcquire(asker2, 250L)
         val endTime = System.currentTimeMillis()
         state.synchronized {
           state.t2Result = result
           // A second call should return 0 because we're now already at 1 / 2N
           state.t2Result2 = manager.tryToAcquire(asker2, 100L)
  ...
  test("releaseMemoryForThisThread") {
    ...
    withSparkContext { sc =>
      ...
      val t2 = startThread("t2") {
        ...
        val endTime = System.currentTimeMillis()
        val r2 = manager.tryToAcquire(asker2, 500L)
        val r3 = manager.tryToAcquire(asker2, 500L)
        state.synchronized {
          state.t2Result1 = r1
          state.t2Result2 = r2
          state.t2Result3 = r3
